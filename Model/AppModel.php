...
class AppModel extends Model
{
	...
	*/
	...
	public function afterSave( $created , $options = array()){
		...
		$usuario = AuthComponent::user('id');
		$modulo = $this->alias;

		if (empty($this->data[$modulo]['nombre'])) {
			$nombre = "Indefinido";
		}
		else{
			$nombre = $this->data[$modulo]['nombre'];
		}

		$nombre = str_replace('/[^\da-z]/i', '', $nombre);

		$dataToSave = array(	
						'modulo'			=> 	$modulo,
						'editado' 			=>	$nombre,
						'accion_id' 		=> 	$accion,
						'administrador_id' 	=> 	$usuario
					);
		
		$this->query("INSERT INTO e_logs (modulo, editado, accion_id, administrador_id) VALUES('" . $modulo ."', '" . $nombre ."', '" .$accion."', '".$usuario."')");
